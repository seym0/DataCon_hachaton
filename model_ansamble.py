...
from sklearn.ensemble import ExtraTreesRegressor, RandomForestRegressor, VotingRegressor, GradientBoostingRegressor
import shap
import pickle
...
# Строим ансамбль из ExtraTreesRegressor, GradientBoostingRegressor, RandomForestRegressor
model = VotingRegressor(
    [('et', ExtraTreesRegressor(n_estimators=170,)),
     ('gbr', GradientBoostingRegressor(n_estimators=1500, learning_rate=0.036, subsample=0.6,)),
     ('rfr', RandomForestRegressor(n_estimators=150,))],
    weights=[0.6, 0.2, 0.2])
scores = cross_val_score(model, shuffled_df_X, shuffled_df_y, cv=5)
...
y_pred = model.predict(X_test) * (max_val - min_val) + min_val
...
plt.title('VotingRegressor: Real vs Predicted (Test Data)')
plt.legend()
plt.show()
...
def weights_model():
    with open("model_ansamble.pkl", "wb") as f:
        pickle.dump(model, f)
